...
function insertService() {
	...
	editor.edit(builder => {
		...
		const originalServiceName = editor.document.getText(
			new vscode.Range(position.line, foundColon + 1, position.line, position.character)
		);
		...
		builder.replace(
			new vscode.Range(position.line, position.character - originalServiceName.length - 1, position.line, position.character),
			replacementText
		);
